...
RUN apt update \
    ...
    && curl -s https://get.sdkman.io | bash && source "/root/.sdkman/bin/sdkman-init.sh" && sdk install kotlin \
    && curl https://storage.googleapis.com/flutter_infra/releases/stable/linux/flutter_linux_1.22.6-stable.tar.xz -o flutter-sdk.tar.xz && tar xf flutter-sdk.tar.xz && rm flutter-sdk.tar.xz
